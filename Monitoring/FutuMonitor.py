...
class KLineHandler(StockQuoteHandlerBase):
    ...
    def on_recv_rsp(self, rsp_str):
        ...
        for row in data.itertuples():
            ...
            # 增量计算
            try:
                self.monitor.chan_objects[code].append_kl(new_kl)
                # 检查是否有新买卖点
                latest_bsp = self.monitor.chan_objects[code].get_latest_bsp(number=1)
                if latest_bsp and self.monitor.ui_callback:
