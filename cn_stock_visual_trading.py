...
class CNStockVisualTrading:
    ...
    def analyze_with_chan(self, code: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(f"{code} {bsp.type2str()} 信号在 4 小时窗口内")
            return {
                'code': code,
    ...
    def generate_charts(self, code: str, chan_multi_level: CChan) -> List[str]:
        ...
        try:
            for i, lv in enumerate([KL_TYPE.K_30M, KL_TYPE.K_5M]):
                plot_driver = CPlotDriver(chan_multi_level[i], plot_config={
    ...
    def _batch_generate_charts(self, candidate_signals: List[Dict]) -> List[Dict]:
        ...
        signals_with_charts = []
        max_workers = min(4, len(candidate_signals))
        if max_workers == 0: return []

        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            future_to_signal = {executor.submit(self._generate_single_chart, s): s for s in candidate_signals}
