...
class CFutuAPICached(CCommonStockApi):
    def __init__(self, code, k_type, begin_date=None, end_date=None, autype=AUTYPE.QFQ):
        super(CFutuAPICached, self).__init__(code, k_type, begin_date, end_date, autype)
    ...
    def get_kl_data(self):
        """
        [EvoMap Strategy Fusion - Online Version with Cache]
        ...
        - Added: Cache support to avoid repeated API calls
        """
        ...
        # 尝试从缓存获取数据
        cached_df = kline_raw_cache.get_cached_data(
            stock_code, self.k_type, self.begin_date, self.end_date
        )
        
        if cached_df is not None:
            print(f"✅ 从缓存获取数据: {stock_code} {self.k_type.name}")
            # 从缓存的DataFrame生成CKLine_Unit
        ...
        try:
            ...
            if ret == RET_OK and not data.empty:
                # 保存到缓存
                kline_raw_cache.save_data_to_cache(
                    stock_code, self.k_type, self.begin_date, self.end_date, data
                )
                print(f"✅ 从API获取并缓存数据: {stock_code} {self.k_type.name}")
