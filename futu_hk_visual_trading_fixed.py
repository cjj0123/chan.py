...
class FutuHKVisualTrading:
    ...
    def generate_charts(self, code: str, chan_analysis: Dict) -> List[str]:
        """
        生成技术图表（AI视觉优化版）- 修复版：分别生成30M和5M图表
        """
        chart_paths = []
        ...
        with self.chart_generation_lock:
            try:
                ...
                if chan_30m:
                    ...
                    logger.info(f"✅ 生成30M图表: {chart_path_30m}")
                ...
                if chan_5m:
                    ...
                    logger.info(f"✅ 生成5M图表: {chart_path_5m}")
                else:
                    logger.warning(f"{code} 5M数据不可用，仅生成30M图表")
        ...
        if buy_signals:
            ...
            for i, signal in enumerate(buy_signals, 1):
                ...
                if stocks_bought >= max_stocks_to_buy:
                    logger.info(f"✅ 已达到最大买入股票数量限制({max_stocks_to_buy}只)，停止买入")
                    break
                ...
                if available_funds < 50000:
                    logger.info(f"💰 可用资金({available_funds:.2f})少于5万元，停止买入操作")
                    break
                ...
                if score >= self.min_visual_score:
                    ...
                    if lots_can_buy < 1:
                        logger.warning(f"[{i}/{len(buy_signals)}] {code} 资金不足，无法购买一手 (需要: {price * lot_size:.2f}, 可用份额: {investment_amount:.2f})")
                        continue
                    ...
                    if required_funds > available_funds:
                        logger.warning(f"[{i}/{len(buy_signals)}] {code} 资金不足，需要{required_funds:.2f}，可用{available_funds:.2f}，跳过")
                        continue
                    
                    logger.info(f"\n[{i}/{len(buy_signals)}] 买入 {code} - {bsp_type} - 评分: {score}")
                    logger.info(f"   计划买入: {buy_quantity}股 ({lots_can_buy}手), 预计花费: {required_funds:.2f} (分配份额: {investment_amount:.2f})")
                    ...
                    if self.execute_trade(code, 'BUY', buy_quantity, price):
                        ...
                        logger.info(f"✅ 买入成功 {code}, 剩余资金: {available_funds:.2f}, 已买入{stocks_bought}/{max_stocks_to_buy}只")
                    else:
...
def main():
    ...
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='港股缠论视觉交易系统')
    parser.add_argument('--freq', type=str, default='30m', help='K线周期 (30m, 5m)')
    parser.add_argument('--tag', type=str, default='core_scan', help='扫描标签 (core_scan, open_predict, close_predict, cas_predict, cas_execute, final_confirm)')
    ...
    try:
        ...
        # 只有特定标签才启用真实交易，其他都为模拟模式
        if args.tag in ['cas_execute', 'final_confirm']:
            dry_run = False  # 启用真实交易
        else:
            dry_run = True   # 模拟模式

        # 初始化交易系统
        trader = FutuHKVisualTrading(dry_run=dry_run)
        
        # 根据标签执行不同的操作
        if args.tag in ['core_scan_am', 'core_scan_pm', 'core_scan']:
            # 核心扫描：执行完整扫描和交易逻辑
            trader.scan_and_trade()
        elif args.tag in ['open_predict', 'close_predict', 'cas_predict']:
            # 预测扫描：可能只进行分析不执行交易
            trader.scan_and_trade()
        elif args.tag in ['cas_execute', 'final_confirm']:
            # 执行扫描：执行交易
            trader.scan_and_trade()
        else:
            # 默认行为
