...
def test_seg_chart_generation():
    ...
    print(f"缠论分析完成，检测到:")
    print(f"  笔数量: {len(chan_30m.kl_datas[0].bi_list)}")
    print(f"  线段数量: {len(chan_30m.kl_datas[0].seg_list)}")
    print(f"  中枢数量: {len(chan_30m.kl_datas[0].zs_list)}")
    ...
    plot_30m = CPlotDriver(
        ...
        plot_para={
            ...
            "seg": {
                "color": "#FF0000"   # 红色线段（移除linewidth参数）
            },
            "zs": {
                "color": "#4169E1",  # 皇家蓝中枢
                "linewidth": 2
