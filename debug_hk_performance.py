...
class HKPerformanceAnalyzer:
    ...
    def run_comprehensive_analysis(self):
        ...
        test_code = watchlist[0]  # 使用第一个股票进行详细分析
        ...
        logger.info(f"  分别获取总时间: {kline_results['30m']['time'] + kline_results['5m']['time']:.2f}s")
        ...
        if kline_results['both']['success']:
            ...
            logger.info(f"  理论并行时间: max({kline_results['30m']['time']:.2f}, {kline_results['5m']['time']:.2f}) = {max(kline_results['30m']['time'], kline_results['5m']['time']):.2f}s")
            logger.info(f"  预计节省: {(total_separate - max(kline_results['30m']['time'], kline_results['5m']['time']))/total_separate*100:.1f}% 时间")
