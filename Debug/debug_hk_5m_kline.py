...
def test_hk_5m_kline_direct(code: str, days: int = 30):
    """
    直接使用Futu API测试5分钟K线获取
    """
    print(f"=== 测试港股 {code} 5分钟K线获取 ===")
...
def test_hk_30m_kline_direct(code: str, days: int = 30):
    """
    直接使用Futu API测试30分钟K线获取（作为对比）
    """
    print(f"\n=== 测试港股 {code} 30分钟K线获取 ===")
    ...
    try:
        ...
        ret, data = quote_ctx.subscribe([code], [SubType.K_30M])
        if ret != RET_OK:
            print(f"❌ 订阅失败: {data}")
            return False
        ...
        if ret == RET_OK:
            print(f"✅ 获取成功！共 {len(kline_data)} 根30分钟K线")
            if len(kline_data) > 0:
                print(f"最新K线时间: {kline_data.iloc[-1]['time_key']}")
                print(f"最新K线价格: {kline_data.iloc[-1]['close']}")
                return True
...
def test_with_cchan(code: str, days: int = 30):
    """
    使用CChan测试5分钟K线获取
    """
    print(f"\n=== 使用CChan测试港股 {code} 5分钟K线获取 ===")
    ...
    try:
        ...
        # 统计K线数量
        kline_5m_count = 0
        for _ in chan_5m.get_sub_level(KL_TYPE.K_5M).kl_list():
            kline_5m_count += 1
...
if __name__ == "__main__":
    # 测试几个典型的港股代码
    test_codes = ["HK.00700", "HK.00966", "HK.00916"]
    ...
    for code in test_codes:
        ...
        # 测试30分钟K线（应该正常工作）
        success_30m = test_hk_30m_kline_direct(code)
        
        # 测试5分钟K线（可能有问题）
        success_5m = test_hk_5m_kline_direct(code)
        
        # 测试CChan方式
        success_cchan = test_with_cchan(code)
        
        print(f"\n📊 测试结果汇总 - {code}:")
        print(f"   30分钟K线: {'✅' if success_30m else '❌'}")
        print(f"   5分钟K线: {'✅' if success_5m else '❌'}")
